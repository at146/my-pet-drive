...
BOT_TOKEN:
"8051112543:AAFL3Slv1q1NadBxAIq29gefPWWd5dNGh_Y",
...
try {
const r = await fetch(
    `${CONFIG.SHEETDB}/search?order_code=${orderCode}`
);
...
const validationResult = validateOrderStatus(
    order.status
);
...
document.getElementById("status-error-text").textContent =
message;
document
.getElementById("status-error")
.classList.remove("hidden");
document
.getElementById("order-card")
.classList.add("hidden");
document
.getElementById("bid-form-container")
.classList.add("hidden");
}
...
document.getElementById("driver-pay").textContent =
order.driver_cost;
document.getElementById("bid").value = order.driver_cost;

if (order.mssg_cl && order.mssg_cl.trim()) {
document
    .getElementById("client-comment-block")
    .classList.remove("hidden");
document.getElementById("client-comment").textContent =
    order.mssg_cl;
}
...
Promise.all([
geo(order.departure_address),
geo(order.destination_address),
])
.then((c) => {
    markers.forEach((m) => map.removeLayer(m));
    markers = [];
    const m1 = L.marker(c[0])
    .addTo(map)
    .bindPopup("Откуда");
    const m2 = L.marker(c[1])
    .addTo(map)
    .bindPopup("Куда");
...
console.warn(
    "Ошибка автозаполнения данных водителя:",
    e
);
}
}
...
const bid =
    parseFloat(document.getElementById("bid").value) || 0;
const comment = document
    .getElementById("comment")
    .value.trim();
let phone = document
    .getElementById("driver-phone")
    .value.trim();

if (phone.startsWith("+")) phone = phone.substring(1);

const car_brand = document
    .getElementById("driver-car-brand")
    .value.trim();
const car_num = document
    .getElementById("driver-car-num")
    .value.trim();
const car_color = document
    .getElementById("driver-car-color")
    .value.trim();
...
const cost_with_com = Math.round(
    bid * (1 + CONFIG.COMMISSION_RATE)
);
...
try {
    await fetch(
    `${CONFIG.SHEETDB}/order_code/${orderCode}`,
    {
        method: "PATCH",
